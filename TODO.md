...
- [x] [P1][i18n] Проинвентаризировать хардкод‑строки в `src/routes/**/*.svelte` и вынести их в словари. ✅ Реализовано
  - Локализованы: troubleshoot (~40+ ключей), plugins (~40+ ключей)
...
- [x] [P1][UX][A11y] Пройтись по всем основным страницам (Dashboard, Services/Library, Network, Orchestra/Troubleshooter, Plugins, Settings) и проверить контраст в тёмной теме. ✅ Реализовано
  - Критерий: все ключевые тексты соответствуют WCAG AA.
  - Исправлены: text-zinc-600/500 → text-zinc-400, placeholder цвета, убраны opacity, увеличены мелкие шрифты
