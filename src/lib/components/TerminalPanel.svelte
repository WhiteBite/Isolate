...
<button
  onclick={toggle}
  class="fixed bottom-4 right-4 z-50 flex items-center gap-2 px-3 py-2 
         bg-zinc-900/90 backdrop-blur-sm border border-white/5 rounded-lg
         hover:bg-zinc-800 hover:border-white/10
         transition-all duration-200 text-sm font-medium text-zinc-400
...
>
  ...
  <span>Terminal</span>
  <kbd class="px-1.5 py-0.5 text-[10px] bg-zinc-800 rounded border border-white/5 text-zinc-500">Ctrl+`</kbd>
</button>
  ...
  >
    ...
    <div class="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-zinc-900/60">
      <div class="flex items-center gap-4">
        ...
        <!-- Search -->
        <div class="relative">
          <svg class="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-zinc-500" 
        ...
        <button
          ...
          class="flex items-center gap-1.5 px-2 py-1 text-xs rounded
                 ...
                 {autoScroll 
                   ...
                   : 'bg-zinc-800/50 text-zinc-500 border border-white/5 hover:text-zinc-400'}"
          title="Auto-scroll"
        >
    ...
    <div
      ...
      class="flex-1 overflow-y-auto overflow-x-hidden font-mono text-xs bg-zinc-950
    ...
    >
      ...
      {:else}
        {#each $filteredLogs as log (log.id)}
          <div class="flex items-start gap-3 px-4 py-1.5 hover:bg-zinc-900/50 border-b border-white/[0.02]">
            <!-- Timestamp -->
            <span class="text-zinc-600 shrink-0 w-16">
