...
<div class="p-6 space-y-6">
  ...
  <header class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-white">Диагностика</h1>
      <p class="text-zinc-400">Найдём и исправим проблемы с доступом</p>
    </div>
      ...
      <div class="flex items-center gap-2">
        <div 
          class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors
            {isActive ? 'bg-indigo-500 text-white' : isPast ? 'bg-indigo-500/20 text-indigo-400' : 'bg-white/5 text-zinc-500'}"
        >
          {#if isPast}
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <span class="text-sm {isActive ? 'text-white' : 'text-zinc-500'}">
          {#if step === 'select'}
            Выбор проблемы
    ...
    <div class="space-y-4">
      ...
      <div class="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 rounded-2xl border border-indigo-500/20 p-5">
        <div class="flex items-start gap-3 mb-4">
          ...
          <div>
            <h3 class="text-white font-semibold">AI Помощник</h3>
            <p class="text-zinc-400 text-sm">Автоматический подбор стратегии</p>
          </div>
        </div>
        ...
        <div class="space-y-3">
          {#if currentStep === 'select' && !selectedProblem}
            <p class="text-zinc-300 text-sm">
              Выберите сервис, с которым возникли проблемы. Я проанализирую ситуацию и подберу оптимальную стратегию обхода.
            </p>
          {:else if currentStep === 'select' && selectedProblem}
            <p class="text-zinc-300 text-sm">
              Отлично! Вы выбрали <span class="text-indigo-400 font-medium">{selectedProblem.serviceName}</span>. 
              Нажмите "Начать тестирование" для поиска рабочей стратегии.
            </p>
            <button
              type="button"
          ...
          {:else if currentStep === 'testing'}
            <p class="text-zinc-300 text-sm">
              Тестирую стратегии для <span class="text-indigo-400 font-medium">{selectedProblem?.serviceName}</span>...
            </p>
          ...
          {:else if currentStep === 'results'}
            <p class="text-zinc-300 text-sm">
              Тестирование завершено! Выберите рекомендованную стратегию или попробуйте другие варианты.
            </p>
          {/if}
        </div>
      </div>
      ...
      <div class="bg-zinc-900/50 rounded-2xl border border-white/5 p-5">
        ...
        <ul class="space-y-2 text-sm text-zinc-400">
          <li class="flex items-start gap-2">
            ...
            Закройте VPN и прокси перед тестированием
          </li>
          <li class="flex items-start gap-2">
            <span class="text-zinc-500">•</span>
          ...
          <li class="flex items-start gap-2">
            <span class="text-zinc-500">•</span>
            Если ничего не помогает — попробуйте VLESS прокси
          </li>
        </ul>
      </div>
      ...
      <div class="bg-zinc-900/50 rounded-2xl border border-white/5 p-5">
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between text-sm">
            <span class="text-zinc-400">WinDivert</span>
          ...
          <div class="flex items-center justify-between text-sm">
            <span class="text-zinc-400">Интернет</span>
            <span class="text-emerald-400 flex items-center gap-1">
              <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
              Подключен
            </span>
          ...
          <div class="flex items-center justify-between text-sm">
            <span class="text-zinc-400">Права админа</span>
            <span class="text-emerald-400 flex items-center gap-1">
              <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
              Есть
            </span>
      ...
      <div class="bg-zinc-900/50 rounded-2xl border border-white/5 p-5">
        ...
        <p class="text-sm text-zinc-400 mb-3">
          Хотите протестировать все стратегии сразу или настроить параметры оптимизации?
        </p>
        <a 
          href="/orchestra"
